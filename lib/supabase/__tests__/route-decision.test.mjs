...
test("anonymous is sent to login from a private route, but not looped from /login", () => {
	...
	assert.equal(
		decideRoute({ pathname: "/character/123", email: undefined }),
		"login",
	);
...
test("anonymous can reach the not-invited page", () => {
	assert.equal(
		decideRoute({ pathname: "/not-invited", email: undefined }),
		"next",
	);
});
...
test("signed in and invited passes through a private route, and /login sends them home", () => {
	assert.equal(
		decideRoute({ pathname: "/", email: "x@example.com", invited: true }),
		"next",
	);
	assert.equal(
		decideRoute({ pathname: "/login", email: "x@example.com", invited: true }),
		"home",
	);
});
...
test("signed in but not invited is sent to not-invited from anywhere, including /login", () => {
	assert.equal(
		decideRoute({ pathname: "/", email: "x@example.com", invited: false }),
		"not-invited",
	);
	assert.equal(
		decideRoute({ pathname: "/login", email: "x@example.com", invited: false }),
		"not-invited",
	);
	assert.equal(
		decideRoute({
...
test("a signed-in email with no invite check counts as not invited (fail closed)", () => {
	assert.equal(
		decideRoute({ pathname: "/", email: "x@example.com" }),
		"not-invited",
	);
});
