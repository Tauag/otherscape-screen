...
test("editing a set's title touches only that set", () => {
	const next = editLoadoutSetTitle(
		loadout,
		"ls-2",
		"the bone lantern's whole rig",
	);
...
test("loading a title costs Power; unloading it cascades to every loaded feature", () => {
	...
	assert.equal(
		unloaded.sets[1].features[0].loaded,
		false,
...
test("adding, editing, and removing a feature touches only that set", () => {
	...
	const edited = editLoadoutFeature(
		added,
		"ls-1",
		"lf-new",
		"a second cartridge",
...
test("available Power adjusts by hand, clamped at 0", () => {
	assert.equal(
		adjustLoadoutPower(loadout, 1).availablePower,
		loadout.availablePower + 1,
	);
...
test("taking the Upgrade as Power clears the track and adds 1 available Power", () => {
	...
	assert.equal(
		loadoutSpend(taken).available,
		loadoutSpend(loadout).available + 1,
	);
});
