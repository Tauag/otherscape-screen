...
export function burnValueOf(tag: Pick<PowerTag, "burnt" | "burnValue">): number | null {
...
export function power(selection: RollSelection): PowerBreakdown {
  ...
  for (const status of statuses) {
    ...
    const counts = status.tier === best[status.valence] && !counted[status.valence];
  ...
  const total = lines.reduce((sum, line) => (line.counted ? sum + line.value : sum), 0);
  return { lines, total };
}
