...
export type PowerLine = { value: number; counted: boolean };
...
export type SelectedStatus = { valence: Valence; tier: number };
...
/**
 * Break a roll selection into per-tag lines plus the total. `rollOrder` gives
 * the id behind each line, positionally, so the roll screen can show each
 * tag chip its own value and whether it counted, without a separate
 * breakdown printout.
 */
export function power(selection: RollSelection): PowerBreakdown {
	...
	if (rollWith) {
		lines.push({ value: rollWith.themeCount, counted: true });
	}

	for (const tag of tags) {
		if (tag.valence === "negative") {
			lines.push({ value: -1, counted: true });
			continue;
		}
		...
		lines.push({ value: burn ?? 1, counted: !rollWith });
	}
	...
	if (modifier !== 0) {
		lines.push({ value: modifier, counted: true });
	}
