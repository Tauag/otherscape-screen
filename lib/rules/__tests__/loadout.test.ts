...
function set(overrides: Partial<LoadoutSet> = {}): LoadoutSet {
  return { id: "ls-1", title: "kit", titleLoaded: false, features: [], weaknesses: [], ...overrides };
}

function loadout(availablePower: number, overrides: Partial<Loadout> = {}): Loadout {
  return { sets: [], wildcards: 0, specials: [], availablePower, upgrade: 0, ...overrides };
}
...
test("a loaded title costs 1, and a loaded feature costs 1 more", () => {
  ...
  assert.deepEqual(loadoutSpend(l), { spent: 2, available: 3, over: 0, warning: null });
});

test("a weakness never costs anything, loaded title or not", () => {
  const l = loadout(0, {
    sets: [set({ titleLoaded: true, weaknesses: [{ id: "lw-1", text: "flinches at chrome" }] })],
  });
