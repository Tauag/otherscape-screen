...
test("every theme mix maps to its Essences", () => {
  ...
  assert.deepEqual(essenceCandidates(themes("self", "mythos")), ["Spiritualist"]);
  assert.deepEqual(essenceCandidates(themes("noise", "self")), ["Cyborg"]);
  assert.deepEqual(essenceCandidates(themes("mythos", "noise")), ["Transhuman"]);
  assert.deepEqual(essenceCandidates(themes("noise", "self", "mythos")), ["Nexus"]);
