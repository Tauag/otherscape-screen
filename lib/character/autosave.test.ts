...
test("the local copy wins only when it holds edits the server never saw", () => {
  ...
  assert.equal(resolve(entry(4, false), 4), "remote", "stored but already saved");
  assert.equal(resolve(entry(2, false), 4), "remote", "stored, saved, and stale");
  assert.equal(resolve(entry(4, true), 4), "local", "the offline edit");
  assert.equal(resolve(entry(5, true), 4), "local", "a stale read of the server");
