...
test("a tag moves up and down, and the ends hold", () => {
	...
	assert.deepEqual(ids(moveTag(past, "power", "pt-2", "up")), [
		"pt-2",
		"pt-1",
		"pt-3",
	]);
	assert.deepEqual(ids(moveTag(past, "power", "pt-2", "down")), [
		"pt-1",
		"pt-3",
		"pt-2",
	]);
...
test("a question answered twice gives two tags that edit independently", () => {
	...
	assert.equal(
		edited.powerTags.find((tag) => tag.id === "pt-new")?.text,
		"a second favour",
	);
	assert.equal(
		edited.powerTags.find((tag) => tag.id === "pt-2")?.text,
		past.powerTags[1].text,
	);
});
