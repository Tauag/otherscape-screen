...
export function readLocal(id: string): LocalEntry | null {
  ...
  try {
    ...
    const { version, dirty, savedAt, document } = entry as Record<string, unknown>;
...
export const writeLocal = (id: string, entry: LocalEntry) => writeRaw(localKey(id), entry);
 ...
 */
export function resolve(local: LocalEntry | null, remoteVersion: number): "local" | "remote" | "conflict" {
