  ...
  themebooks: [
    {
      ...
      name: "X",
      type: "Self",
      power_tag_questions: Object.fromEntries(POWER_LETTERS.map((letter) => [letter, "q"])),
      weakness_tag_questions: Object.fromEntries(WEAKNESS_LETTERS.map((letter) => [letter, "q"])),
...
function assertShape(pack: ContentPack) {
  for (const book of pack.themebooks) {
    assert.ok(["self", "mythos", "noise"].includes(book.type), `${book.id} has type ${book.type}`);
    assert.deepEqual(
...
test("the fallback holds the 14 themebooks, with every text slot empty", () => {
  ...
  for (const book of FALLBACK_PACK.themebooks) {
    ...
    for (const question of [...book.powerQuestions, ...book.weaknessQuestions]) {
...
test("a themebook is found by id or by the free-text name a theme stores", () => {
  assert.equal(findThemebook(FALLBACK_PACK, "troubled-past")?.name, "Troubled Past");
  assert.equal(findThemebook(FALLBACK_PACK, " troubled past ")?.id, "troubled-past");
...
test("the real content pack normalizes into the same shape, with text filled", (t) => {
  ...
  for (const book of pack.themebooks) {
    ...
    for (const question of [...book.powerQuestions, ...book.weaknessQuestions]) {
      assert.ok(question.text.length > 0, `${book.id} ${question.letter} has text`);
    }
...
test("a storage that throws, or holds nonsense, reads as no cache", () => {
  ...
  cache.setItem(PACK_CACHE_KEY, JSON.stringify({ updatedAt: "2026-01-01T00:00:00Z", pack: {} }));
  assert.equal(readCachedPack(cache, "2026-01-01T00:00:00Z"), null);
});
