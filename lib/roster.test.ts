...
test("parseRosterSummary reads the verified shapes", () => {
	assert.deepEqual(parseRosterSummary({ themes: [], statuses: 0 }), {
		themes: [],
		statuses: 0,
	});
	assert.deepEqual(
		parseRosterSummary({
			themes: [{ type: "mythos", nascent: false }],
			statuses: 2,
		}),
...
test("parseRosterSummary never throws on a malformed value", () => {
	for (const value of [
		...
		[],
		{ themes: "nope" },
		{ themes: [1, null], statuses: -1 },
	]) {
		assert.doesNotThrow(() => parseRosterSummary(value));
	}
	assert.deepEqual(parseRosterSummary({ themes: "nope", statuses: "3" }), {
		themes: [],
		statuses: 0,
	});
	assert.deepEqual(
		parseRosterSummary({ themes: [1, null, { type: "noise" }] }),
		{
