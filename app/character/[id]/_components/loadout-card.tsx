...
export function LoadoutCard({ loadout, href }: { loadout: Loadout; href: string }) {
                        ...
                        {(features.length > 0 || set.weaknesses.length > 0) && (
                          <ul className="flex flex-wrap gap-1.5">
                            {features.map((feature) => (
                              <Chip key={feature.id} label="◆" text={feature.text} />
                            ))}
                            {set.weaknesses.map((weakness) => (
                              <Chip key={weakness.id} label="⚠" text={weakness.text} negative />
                            ))}
                          </ul>
                        )}
                      </div>
