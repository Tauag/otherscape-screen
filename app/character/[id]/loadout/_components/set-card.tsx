    ...
    <section className="flex flex-col gap-3 rounded-md border border-border bg-surface p-4">
      <div className="flex items-center gap-2">
        ...
        <Input
          ...
          onChange={(event) =>
            dispatch({ type: "editLoadoutSetTitle", setId: set.id, text: event.target.value })
          }
          ...
          {set.features.map((feature) => (
            ...
            >
              <Toggle
                ...
                onPressedChange={() =>
                  dispatch({ type: "toggleLoadoutFeature", setId: set.id, featureId: feature.id })
                }
                className={LOAD_TOGGLE}
              >
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          type="button"
          onClick={() =>
            dispatch({ type: "addLoadoutFeature", setId: set.id, id: crypto.randomUUID() })
          }
        ...
        <Button
          type="button"
          onClick={() =>
            dispatch({ type: "addLoadoutWeakness", setId: set.id, id: crypto.randomUUID() })
          }
          className={`${SMALL_BUTTON} text-dim`}
        >
          + Weakness
